...
func (r StagedUserResource) CreateUser(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	all := true
	optArgs := ipa.StageuserAddOptionalArgs{}
	args := ipa.StageuserAddArgs{
...
func (r StagedUserResource) UpdateUser(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	all := true
	optArgs := ipa.StageuserModOptionalArgs{}
	optArgs.All = &all
	...
	res, err := r.client.StageuserMod(&ipa.StageuserModArgs{}, &optArgs)
	if err != nil && !strings.Contains(err.Error(), "EmptyModlist") {
		resp.Diagnostics.AddError("Client Error", err.Error())
		return
	}
	tflog.Debug(ctx, fmt.Sprintf("[DEBUG] Update freeipa staged user %s returns %s", data.UID.String(), res.String()))
